"use client";    // for hooks like useEffect and useState
...
import { MessageFeedbackModal } from "@/components/chat/MessageFeedbackModel";    // pops up when we want to like/dislike a particular message
...
export default function ProjectChatPage({ params }: ProjectChatPageProps) {
  ...
  // Send message function (it gets triggered when we type something and click send)
  const handleSendMessage = async (content: string) => {
    try {
      ...
      // Create optimistic user message to show immediately (updates the react state with a temp user message)
        ...
        return {
          ...
          messages: [
            ...prev.messages.filter(
              (msg) => msg.id !== optimisticUserMessage.id    // filter out temp message
            ),
            userMessage,
            aiMessage,
          ],
  ...
  // loads the current chat from the api 
  useEffect(() => {
