  ...
  const handleDocumentUpload = async (files: File[]) => {
    ...
    const uploadPromises = files.map(async (file) => {
      try {
        ...
        // this is for the server to update the processing_status to queued/completed
        ...
        uploadedDocuments.push(updatedDocument.data);    // To update local react state
      } catch (err) {
        toast.error(`Failed to upload ${file.name}`);
      }
    });

    await Promise.allSettled(uploadPromises);    // only after all the API calls related to all the files have settled move to the next code
